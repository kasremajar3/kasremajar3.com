...
function riwayatPendapatan($id_user, $aksi) {
	global $conn;
	$tanggal = time();
	return mysqli_query($conn, "INSERT INTO riwayat_pendapatan VALUES ('', '$id_user', '$aksi', '$tanggal')");
}
...
function addPendapatan($data) {
	...
	$keterangan = htmlspecialchars($data['keterangan']);
	$tanggal_pendapatan = time();
	$query = mysqli_query($conn, "INSERT INTO pendapatan VALUES ('', '$jumlah_pendapatan', '$keterangan', '$tanggal_pendapatan', '$id_user')");
	riwayatPendapatan($id_user, "telah menambahkan pendapatan " . $keterangan . " dengan biaya Rp. " . number_format($jumlah_pendapatan));
  	return mysqli_affected_rows($conn);
}
...
function editPendapatan($data) {
	...
	$fetch_sql = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM pendapatan WHERE id_pendapatan = '$id_pendapatan'"));
	$jumlah_pendapatan = htmlspecialchars($data['jumlah_pendapatan']);
	$keterangan = htmlspecialchars($data['keterangan']);
	$tanggal_pendapatan = time();
	$query = mysqli_query($conn, "UPDATE pendapatan SET jumlah_pendapatan = '$jumlah_pendapatan', keterangan = '$keterangan', tanggal_pendapatan = '$tanggal_pendapatan', id_user = '$id_user' WHERE id_pendapatan = '$id_pendapatan'");
	riwayatPendapatan($id_user, "telah mengubah pendapatan " . $keterangan . " dari biaya Rp. " . number_format($fetch_sql['jumlah_pendapatan']) . " menjadi Rp. " . number_format($jumlah_pendapatan));
  	return mysqli_affected_rows($conn);
}
